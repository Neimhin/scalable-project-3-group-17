...
def find_free_gateway_port_reverse():
    global start_port_reverse
    start_port = start_port_reverse
    for port in range(start_port, START_PORT, -1):
        try:
            # Attempt to bind to the port
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.bind(('', port))
                # move start_port for more efficient search next time
                start_port_reverse = port - 1
...
if __name__ == "__main__":
    ...
    for i in range(100):
        print(find_free_gateway_port_reverse())
