...
class Device:
    ...
    async def handler(self, request):
        ...
        match packet_type:
            case "interest":
                asyncio.create_task(self.handle_interest_packet(packet,hop=hop_count+1))
            case "satisfy":
        ...
        async def handle():
            while True:
                ...
                payload = self.jwt.encode({
                    ...
                    PACKET_FIELD_CREATED_AT: datetime.now().timestamp(),
                    PACKET_FIELD_PORT_NUMBER: self.server.port  #add port number entry, to trace the process of delivery
                })
